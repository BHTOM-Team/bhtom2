...
class PiwnicePhotometryObservationForm(BaseManualObservationForm):
...
class Piwnice(BaseManualObservationFacility):
    name = 'Piwnice'
    SITES = {
        'Piwnice': {
            'sitecode': 'piwnice',
    ...
    observation_forms = {
        'PHOTOMETRIC_SEQUENCE': PiwnicePhotometryObservationForm,
    }
    ...
    def get_facility_weather_urls(self):
        facility_weather_urls = {
            ...
            'sites': [
                {
                    'code': site['sitecode'],
                    'weather_url': 'http://alps.astro.uni.wroc.pl/alps_piw/'
                }
