...
from bhtom_base.bhtom_targets.tests.factories import NonSiderealTargetFactory, SiderealTargetFactory, TargetGroupingFactory, TargetNameFactory
...
class TestTargetImport(TestCase):
    ...
    def test_import_no_type(self):
        csv = [
            'name,ra,dec,redshift',
            'm13,250.421,36.459,5',
            'm27,299.901,22.721,5'
        ]
        result = import_targets(csv)
        self.assertEqual(len(result['targets']), 2)
        for target in result['targets']:
            self.assertTrue(target.type==Target.SIDEREAL)

    # #THIS TEST FAILS as Django forbids to run queries in another transaction.atomic, but it works in the prod code.
    # def test_import_no_type_no_coords(self):
    #     csv = [
    #         'name,GAIA_ALERTS_name,redshift',
    #         'mytarget,Gaia19dke,5',
    #         'myquasar,Gaia19axp,5'
    #     ]
    #     aliases = {'mytarget': [('GAIA_ALERTS', 'Gaia19dke')],
    #         'myquasar': [('GAIA_ALERTS', 'Gaia19axp')]}
