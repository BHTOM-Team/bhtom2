...
from bhtom2.external_service.data_source_information import (TARGET_NAME_KEYS,
                                                             DataSource)
from bhtom2.bhtom_calibration.models import cpcsData
...
def data_product_post_upload(dp, target, observatory_name, observation_filter, MJD, expTime, dry_run, 
                             matchDist, comment, user, priority, plot, facility_name=None, observer_name=None):
    url = settings.MEDIA_ROOT + format(dp)
    logger.info('Running post upload hook for DataProduct: {}'.format(url))

    if dp.data_product_type == 'fits_file' and observatory_name is not None:
        print("TODO connection to ccdphotd")
        # Implement the necessary logic to handle fits_file type and observatory_name
        # ...
         #         try:
    ...
    elif dp.data_product_type == 'photometry' and observatory_name is not None and MJD is not None and expTime is not None:
        try:
            instance = cpcsData.objects.create(
                dataproduct_id_id=dp.id,
                status_message='Sent to Calibration',
                created=datetime.now(),
                mjd=MJD,
                expTime=expTime,
                match_distans=matchDist,
                comment=comment,
                useCatalog_id=1,
                noPlot=plot
            )
            send_to_cpcs(instance.id)
...
def send_to_cpcs(id):
    url_cpcs = settings.CPCS_BASE_URL + 'uploadFileId/'
    logger.info('Send file to cpcs: ' + str(id))
    response = requests.post(url_cpcs, {'fileId': id})
    if response.status_code == 201:
        print("OK")
    else:
        print("EEEERRRROOOOR")