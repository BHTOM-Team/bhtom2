...
def input_arguments():
    global indir, intoken, inobject, infilter, intype, dryrun, inmjd, matching_radius
    ...
    parser.add_argument("-d", "--dir", type=str, help="directory containing FITS images or a PHOT filepath",
                        required=True)
    parser.add_argument("-ht", "--token", type=str, help="your dedicated token", required=True)
    ...
    parser.add_argument("--dryrun", help="sends data, but does not store datapoints in BHTOM database",
                        action="store_true")
...
def send_fits_file(filename, token, target, flter, m_radius, dry_run):
...
if __name__ == '__main__':
    ...
    for filename in os.listdir(indir):
        ...
        print(prompt, end="\r")
        msg, code = send_fits_file(filename, token, inobject, infilter, matching_radius, dryrun)
        if code == 200:
            success += 1
