...
class ReloadFits(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        data_ids = request.POST.getlist('selected-target')
        user = request.user
...
class ReloadPhotometry(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        ...
        for data_id in data_ids:
            ...
            calib = Calibration_data.objects.get(dataProduct=data_id)
            calib.status = "C"
            calib.save()

            CalibCreateEventProducer().send_message(data_id, dataProduct.target, dataProduct.data)
