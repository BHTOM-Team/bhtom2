...
class twomassBrokerQueryForm(GenericQueryForm):
...
class twomassBroker(BHTOMBroker):
    name = 'twomass'  # Add the DataSource.XXX.name here -- DataSource is an enum with all possible sources of data
    # bhtom2.external_service.data_source_information

    form = twomassBrokerQueryForm

    def __init__(self):
        super().__init__(DataSource.twomass)  # Add the DataSource here
    ...
    def process_reduced_data(self, target, alert=None) -> Optional[LightcurveUpdateReport]:
        ...
        hasName = ""
        try:
            hasName = TargetExtra.objects.get(target=target, key=TARGET_NAME_KEYS[DataSource.twomass]).value
        except:
            hasName = ""
        
        #extracts the data only if there is no CRTS name
        if (hasName!="" and self.__update_cadence == None):
            self.logger.debug(f'twomass data already downloaded. Skipping. {target.name}')
            return return_for_no_new_points()

        # Change the log message
        self.logger.debug(f'Updating twomass lightcurve for target: {target.name}')

        ## THIS IS WRONG BELOW! THIS CODE IS JUST PLACEHOLDER, use/edit the one in Harvester!
        sqltwomass=("""Select gpsfmag, rpsfmag, ipsfmag, zpsfmag, gpsfmagerr, rpsfmagerr, ipsfmagerr, zpsfmagerr, gepoch, repoch, iepoch, zepoch, objid from panstarrs_dr1.stackobjectthin 
        WHERE (ginfoflag3&panstarrs_dr1.detectionflags3('STACK_PRIMARY'))>0 AND q3c_radial_query(ra, dec, %f, %f, %f/3600.);""")%(ra, dec,radius)
        twomassres=[]
        try:
            twomassres=WSDBConnection().run_query(sqltwomass)
        except Exception as e:
            self.logger.error(f'Error with WSDB connection for twomass for {target.name}: {e}')

        # Process the data here to obtain a numpy array, list, or whatever feels comfortable to process
        data: List[Tuple] = twomassres
...
#     gpsfmag, rpsfmag, ipsfmag, zpsfmag, gpsfmagerr, rpsfmagerr, ipsfmagerr, zpsfmagerr, gepoch, repoch, iepoch, zepoch , objid
        try:
            ...
            inventName: Optional[str] = "twomass_"+obj_str
            TargetExtra.objects.update_or_create(target=target,
                                            key=TARGET_NAME_KEYS[DataSource.twomass],
                                            defaults={
                                                'value': inventName
                                            })
            
            self.logger.info(f"twomass data found for {target.name}. Downloaded and stored as {inventName}")
            ...
            with transaction.atomic():
                ...
                self.logger.info(f"twomass Broker returned {new_points} points for {target.name}")
        except Exception as e:
