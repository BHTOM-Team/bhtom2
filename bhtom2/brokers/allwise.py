...
class ALLWISEBrokerQueryForm(GenericQueryForm):
...
class ALLWISEBroker(BHTOMBroker):
    name = "ALLWISE"

    form = ALLWISEBrokerQueryForm

    def __init__(self):
        super().__init__(DataSource.ALLWISE)  # Add the DataSource here

        # If the survey is e.g. a space survey, fill the facility and observer names in and treat is as a constant
        self.__FACILITY_NAME: str = "ALLWISE"
        self.__OBSERVER_NAME: str = "ALLWISE"
    ...
    def process_reduced_data(self, target, alert=None) -> Optional[LightcurveUpdateReport]:
        ...
        query = "https://irsa.ipac.caltech.edu/cgi-bin/Gator/nph-query?catalog=allwise_p3as_mep&spatial=cone&radius=5&radunits=arcsec&objstr="+ra_str+"+"+dec_str+"&outfmt=1&selcols=ra,dec,mjd,w1mpro_ep,w1sigmpro_ep,w2mpro_ep,w2sigmpro_ep"
        ...
        try:
            ...
            self.logger.warning(f'Warning: ALLWISE returned no observations (no coverage) for {target.name}')
            return return_for_no_new_points()
        ...
        try:
            ...
            with transaction.atomic():
                ...
                self.logger.info(f"ALLWISE Broker returned {new_points} points for {target.name}")
