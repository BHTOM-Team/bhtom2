...
### Swift UVOT
We retrieved the UVOT data from the HEASARC Data Archive ([https://heasarc.gsfc.nasa.gov/cgi-bin/W3Browse/w3browse.pl](https://heasarc.gsfc.nasa.gov/cgi-bin/W3Browse/w3browse.pl)) and analyzed it using `HEASoft` ([https://heasarc.gsfc.nasa.gov/docs/software/lheasoft/](https://heasarc.gsfc.nasa.gov/docs/software/lheasoft/)) version 6.36 with UVOT `CALDB` version 20240201 ([https://swift.gsfc.nasa.gov/caldb/](https://swift.gsfc.nasa.gov/caldb/)). The source magnitude was extracted using the standard `UVOTSOURCE` ([https://www.swift.ac.uk/analysis/uvot/mag.php](https://www.swift.ac.uk/analysis/uvot/mag.php)) pipeline with a 5 arcsec circular region centered on the source, while the background magnitude was obtained from an annulus with an inner radius of 6.0 arcsec and an outer radius of 12.0 arcsec centered on the source. We considered a point as a detection when the detection sigma (`NSIGMA`) is more than 3. We note the magnitude in the Vega system, which is provided by the `UVOTSOURCE` pipeline.
